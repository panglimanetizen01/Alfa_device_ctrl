        ...
        jint cell_height)
{
    ...
    if (size > 0) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (!arg_java_string) {
                ...
                free(argv);
                return throw_runtime_exception(env, "Null argv element");
            }
    ...
    if (size > 0) {
        ...
        for (int i = 0; i < size; ++i) {
            ...
            if (!env_java_string) {
                ...
                if (argv) { for (char** tmp = argv; *tmp; ++tmp) free(*tmp); free(argv); }
                return throw_runtime_exception(env, "Null environment element");
            }
    ...
    if (!cmd || !cwd || !processIdArray) {
        if (envp) { for (char** tmp = envp; *tmp; ++tmp) free(*tmp); free(envp); }
        if (argv) { for (char** tmp = argv; *tmp; ++tmp) free(*tmp); free(argv); }
        return throw_runtime_exception(env, "createSubprocess() received null required argument");
    }
