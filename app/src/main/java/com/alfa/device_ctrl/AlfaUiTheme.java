...
public final class AlfaUiTheme {
    ...
    public static void apply(Activity activity) {
        ...
        root.setBackgroundColor(OBSIDIAN_CANVAS); float density = activity.getResources().getDisplayMetrics().density; applyTree(root, density); root.post(() -> adaptRuntimeDashboard(root, density));
    }
    ...
    private static void styleButton(Button button, float density, int min) {
        ...
        button.setTextSize(10); button.setTypeface(Typeface.create("sans-serif-medium", Typeface.NORMAL)); button.setTextColor(filled ? OBSIDIAN_CANVAS : textColor); button.setMinimumHeight(Math.max(button.getMinimumHeight(), min)); button.setMinimumWidth(Math.max(button.getMinimumWidth(), min)); button.setPadding(dp(8, density), dp(4, density), dp(8, density), dp(4, density)); button.setAllCaps(false);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) { button.setElevation(0f); button.setStateListAnimator(null); } button.setBackground(controlBackground(accent, density, filled));
    }
    private static void styleText(TextView text, int min) {
        ...
        if (text.isClickable()) text.setMinimumHeight(Math.max(text.getMinimumHeight(), min)); Typeface current = text.getTypeface(); if (current != null) text.setTypeface(Typeface.create(current, current.isBold() ? Typeface.BOLD : Typeface.NORMAL));
    ...
    private static void styleContainer(View view, float density) {
        if (!(view.getBackground() instanceof ColorDrawable)) return; int color = ((ColorDrawable) view.getBackground()).getColor(); int normalized = normalizeSurfaceColor(color); if (normalized == OBSIDIAN_CANVAS) view.setBackgroundColor(OBSIDIAN_CANVAS); else view.setBackground(surfaceBackground(normalized, density));
    }
    ...
    private static void adaptRuntimeDashboard(View root, float density) {
        LinearLayout dashboard = findRuntimeDashboard(root); if (dashboard == null || dashboard.getHeight() <= 0) return; int runtimeCount = RuntimeRegistry.all().size(); if (runtimeCount <= 0) return; int padding = dashboard.getPaddingTop() + dashboard.getPaddingBottom(); int separatorHeight = Math.max(1, dp(3, density)); int separators = Math.max(0, runtimeCount - 1); int available = Math.max(0, dashboard.getHeight() - padding - separators * separatorHeight); int cardHeight = Math.max(dp(48, density), available / runtimeCount); int cards = 0;
