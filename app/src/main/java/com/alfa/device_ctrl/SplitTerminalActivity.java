...
public final class SplitTerminalActivity extends Activity implements RuntimeSessionManager.Listener {
    ...
    @Override protected void onCreate(Bundle state){super.onCreate(state);registry=RuntimeSessionRegistry.get(this);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setBackgroundColor(BG);root.setPadding(dp(8),dp(8),dp(8),dp(8));TextView title=text("SPLIT TERMINAL / DUAL LIVE PTY",PRIMARY,15,true);root.addView(title,new LinearLayout.LayoutParams(-1,dp(44)));status=text("CAPACITY="+registry.capacity()+"\nEach pane owns an independent PTY session.",MUTED,10,false);status.setTypeface(android.graphics.Typeface.MONOSPACE);root.addView(status,new LinearLayout.LayoutParams(-1,dp(50)));LinearLayout panes=new LinearLayout(this);panes.setOrientation(LinearLayout.HORIZONTAL);panes.setWeightSum(2f);left=pane("LEFT","split-left");right=pane("RIGHT","split-right");panes.addView(left.root,new LinearLayout.LayoutParams(0,0,1));panes.addView(right.root,new LinearLayout.LayoutParams(0,0,1));root.addView(panes,new LinearLayout.LayoutParams(-1,0,1));LinearLayout controls=new LinearLayout(this);Button stop=button("STOP ALL",MUTED,v->{registry.stopAll();left.clear();right.clear();renderStatus();});controls.addView(stop,new LinearLayout.LayoutParams(0,dp(52),1));root.addView(controls);setContentView(root);renderRuntimeChoices(left);renderRuntimeChoices(right);}
