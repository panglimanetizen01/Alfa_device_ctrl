...
/** Android-side handoff from a current Gate 6 bootstrap artifact to Gate 7 launch state. */
public final class Gate6ImportActivity extends Activity {
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        heading.setText("Alfa Device Ctrl — Gate 6 → Gate 7\n\n"
                + "Pilih bootstrap.txt dari artefak pipeline current-run. "
                + "Hanya Gate 6 PASS + AUTHORIZED dengan provenance lengkap yang diterima.");
        ...
        Button openAlfa = new Button(this);
        openAlfa.setText("OPEN ALFA OS");
        openAlfa.setOnClickListener(v -> startActivity(new Intent(this, MainActivity.class)));
        root.addView(openAlfa);
    ...
    @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        try {
            ...
            Gate6LaunchContract.importBootstrap(incoming, launch, "ubuntu");
            if (!Gate6LaunchContract.verify(launch, "ubuntu")) throw new IllegalStateException("Gate 7 verification failed after publish");
            if (!incoming.delete()) throw new IllegalStateException("incoming bootstrap cleanup failed");
            evidence.setText("GATE6_IMPORT=PASS\nGATE7_LAUNCH=READY\nRUNTIME=ubuntu\n"
                    + "SOURCE=gate6-bootstrap\nLAUNCH_CONTRACT=" + launch.getAbsolutePath());
