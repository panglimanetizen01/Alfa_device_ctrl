...
public final class AlfaSettingsStore {
    ...
    public String getRuntimeDirectoryOverrides(String fallback){return getString(RUNTIME_DIRECTORY_OVERRIDES,fallback);} public void setRuntimeDirectoryOverrides(String value){setString(RUNTIME_DIRECTORY_OVERRIDES,value);} public void setRuntimeDirectoryOverridesBlocking(String value){setStringBlocking(RUNTIME_DIRECTORY_OVERRIDES,value);}
    ...
    private <T>void updateBlocking(Preferences.Key<T> key,T value){try{store.updateDataAsync(current->{MutablePreferences mutable=current.toMutablePreferences();mutable.set(key,value);return mutable.toPreferences();}).get();}catch(InterruptedException e){Thread.currentThread().interrupt();throw new IllegalStateException("settings-update-interrupted",e);}catch(Exception e){throw new IllegalStateException("settings-update-failed",e);}}
    private void setString(Preferences.Key<String> key,String value){if(value!=null)update(key,value);} private void setStringBlocking(Preferences.Key<String> key,String value){if(value!=null)updateBlocking(key,value);} private void setInt(Preferences.Key<Integer> key,int value){update(key,value);} private void setBoolean(Preferences.Key<Boolean> key,boolean value){update(key,value);}
}
