...
public final class StitchOperationalActivityV2 extends Activity implements RuntimeSessionManager.Listener {
    ...
    private TextView systemState, sessionState, terminalState, telemetry, status;
    ...
    @Override protected void onCreate(Bundle state) {
        ...
        setContentView(build()); active = true; refreshNodes();
    }
    ...
    @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) { super.onActivityResult(requestCode,resultCode,data); if (requestCode == GATE6 && resultCode == RESULT_OK) startSession(); }

    private View build() {
        LinearLayout root = col(BG); root.addView(header(), new LinearLayout.LayoutParams(-1,dp(76))); root.addView(topStatus(), new LinearLayout.LayoutParams(-1,dp(38)));
        FrameLayout pages = new FrameLayout(this); pages.addView(buildNodes()); pages.addView(buildLanes()); pages.addView(buildDiagnostics()); root.addView(pages,new LinearLayout.LayoutParams(-1,0,1));
        root.addView(bottomNav(),new LinearLayout.LayoutParams(-1,dp(58))); return root;
    }
    ...
    private View buildNodes(){ nodePage=col(BG); nodePage.addView(window("NODES // LINUX RUNTIMES",runtimeWindowContent(),"nodes")); nodePage.addView(sp(8)); nodePage.addView(window("NODE PROVENANCE",provenanceContent(),"prov")); return nodePage; }
    private View buildLanes(){ lanePage=col(BG); lanePage.setVisibility(View.GONE); lanePage.addView(window("LANE 01 // INTERACTIVE PTY",terminalWindowContent(),"terminal"),new LinearLayout.LayoutParams(-1,0,1)); lanePage.addView(sp(8)); lanePage.addView(window("LANE 02 // EXECUTION",executionContent(),"execution")); return lanePage; }
    private View buildDiagnostics(){ diagPage=col(BG); diagPage.setVisibility(View.GONE); diagPage.addView(window("DIAGNOSTICS // LIVE EVIDENCE",diagnosticContent(),"diag")); diagPage.addView(sp(8)); diagPage.addView(window("STATE MACHINE",stateContent(),"state")); return diagPage; }

    private View window(String title, View content, String id){
        LinearLayout w=col(SURFACE); w.setBackground(round(SURFACE,8,Color.rgb(60,72,67)));
        LinearLayout bar=row(HIGH); bar.setGravity(Gravity.CENTER_VERTICAL); bar.setPadding(dp(8),0,dp(5),0); bar.addView(txt(title,GREEN,10,true),new LinearLayout.LayoutParams(0,dp(40),1));
        Button min=btn("—",MUTED,null); min.setOnClickListener(v->toggleWindowContent(content,min)); Button max=btn("□",CYAN,null); max.setOnClickListener(v->toggleMax(w,content,max)); Button close=btn("×",RED,v->w.setVisibility(View.GONE));
        bar.addView(min,new LinearLayout.LayoutParams(dp(40),dp(40))); bar.addView(max,new LinearLayout.LayoutParams(dp(40),dp(40))); bar.addView(close,new LinearLayout.LayoutParams(dp(40),dp(40))); w.addView(bar);
        w.addView(content,new LinearLayout.LayoutParams(-1,0,1)); return w;
    }
    ...
    private View stateContent(){ LinearLayout b=col(SURFACE); b.setPadding(dp(10),dp(8),dp(10),dp(8)); b.addView(txt("READY ⇒ OPEN only. INSTALL is structurally absent while READY.",GREEN,10,true)); b.addView(txt("NOT_INSTALLED / INSTALLED / PARTIAL / VERIFYING / FAILED ⇒ INSTALL.",MUTED,10,false)); return b; }
    ...
    private View runtimeCard(RuntimeProfile p){ RuntimeUiState.Status s=runtimeStatus(p); LinearLayout c=row(SURFACE); c.setGravity(Gravity.CENTER_VERTICAL); c.setPadding(dp(8),0,dp(4),0); c.addView(txt(p.displayName()+"\n"+p.id()+"  "+p.version(),TEXT,10,true),new LinearLayout.LayoutParams(0,dp(54),1)); c.addView(txt(RuntimeUiState.label(s),s==RuntimeUiState.Status.READY?GREEN:MUTED,9,true),new LinearLayout.LayoutParams(dp(78),dp(54))); c.addView(btn("CHECK",MUTED,v->check(p)),new LinearLayout.LayoutParams(dp(62),dp(46))); if(RuntimeUiState.canOpen(s)) c.addView(btn("OPEN",GREEN,v->{selectedRuntime=p;startSession();}),new LinearLayout.LayoutParams(dp(66),dp(46))); else {Button install=btn("INSTALL",GREEN,v->{selectedRuntime=p;install(p);}); install.setEnabled(s!=RuntimeUiState.Status.VERIFYING); c.addView(install,new LinearLayout.LayoutParams(dp(72),dp(46)));} return c; }
    ...
    private void restart(){stop();new android.os.Handler().postDelayed(this::startSession,250);}
    private void stop(){if(sessionManager!=null)sessionManager.stop(); if(sessionState!=null)sessionState.setText("PTY: OFFLINE");}
    ...
    private View txt(String s,int c,int z,boolean bold){TextView t=new TextView(this);t.setText(s);t.setTextColor(c);t.setTextSize(z);t.setGravity(Gravity.CENTER_VERTICAL);if(bold)t.setTypeface(Typeface.MONOSPACE,Typeface.BOLD);return t;}
    ...
    private LinearLayout row(int c){LinearLayout l=new LinearLayout(this);l.setOrientation(LinearLayout.HORIZONTAL);l.setBackgroundColor(c);return l;} private LinearLayout col(int c){LinearLayout l=new LinearLayout(this);l.setOrientation(LinearLayout.VERTICAL);l.setBackgroundColor(c);return l;}
    private View sp(int h){View v=new View(this);v.setLayoutParams(new LinearLayout.LayoutParams(-1,dp(h)));return v;}
    private int dp(int v){return Math.round(v*getResources().getDisplayMetrics().density);} private GradientDrawable round(int c,int r,int stroke){GradientDrawable d=new GradientDrawable();d.setColor(c);d.setCornerRadius(dp(r));d.setStroke(dp(1),stroke);return d;}
    private String id(){return UUID.randomUUID().toString().replace("-","").substring(0,12);}
