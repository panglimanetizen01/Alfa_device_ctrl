...
public final class AlfaApplication extends Application {
    ...
    @Override public void onCreate() {
        ...
        registerActivityLifecycleCallbacks(new ActivityLifecycleCallbacks() {
            ...
            @Override public void onActivityCreated(Activity activity, Bundle state) { installWindowMetricsAndInsetsPolicy(activity); AlfaUiTheme.apply(activity); AlfaFinalUiPresentation.apply(activity); AlfaFinalUiHooks.apply(activity); StitchV1ReferenceConsole.installEntry(activity); }
            @Override public void onActivityResumed(Activity activity) { AlfaUiTheme.apply(activity); AlfaFinalUiPresentation.apply(activity); AlfaFinalUiHooks.apply(activity); StitchV1ReferenceConsole.installEntry(activity); RuntimeStartupCoordinator.onActivityResumed(activity); }
    ...
    private static void validate(File file) throws Exception {
        ...
        if (!Gate6LaunchContract.OUTPUT_SCHEMA.equals(p.getProperty("schema_version"))) throw new IllegalStateException("invalid-schema-version"); if (!"gate7".equals(p.getProperty("gate"))) throw new IllegalStateException("invalid-gate"); if (!"READY".equals(p.getProperty("gate_status"))) throw new IllegalStateException("invalid-gate-status"); if (!"AUTHORIZED".equals(p.getProperty("launch_status")) || !"AUTHORIZED".equals(p.getProperty("authorization_status"))) throw new IllegalStateException("invalid-authorization-status"); if (!"gate6-bootstrap".equals(p.getProperty("launch_source"))) throw new IllegalStateException("invalid-launch-source"); if (RuntimeRegistry.get(p.getProperty("runtime_id")) == null) throw new IllegalStateException("unsupported-runtime-id"); if (!RuntimeRegistry.CANONICAL_REGISTRY_SHA256.equalsIgnoreCase(p.getProperty("runtime_registry_sha256"))) throw new IllegalStateException("stale-runtime-registry"); if (!p.getProperty("source_commit").matches("[0-9a-fA-F]{40}")) throw new IllegalStateException("invalid-source-commit"); if (!p.getProperty("implementation_commit").matches("[0-9a-fA-F]{40}")) throw new IllegalStateException("invalid-implementation-commit"); if (!p.getProperty("gate4_contract_sha256").matches("[0-9a-fA-F]{64}")) throw new IllegalStateException("invalid-gate4-hash"); if (!p.getProperty("profile_sha256").matches("[0-9a-fA-F]{64}")) throw new IllegalStateException("invalid-profile-hash"); if (!p.getProperty("runtime_registry_sha256").matches("[0-9a-fA-F]{64}")) throw new IllegalStateException("invalid-runtime-registry-hash");
    }
    private static void require(Properties p, String key) { if (p.getProperty(key) == null || p.getProperty(key).trim().isEmpty()) throw new IllegalStateException("missing-" + key); }
}
