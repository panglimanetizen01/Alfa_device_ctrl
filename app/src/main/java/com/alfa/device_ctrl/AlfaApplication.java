...
/** Canonical application lifecycle, runtime reattachment, edge-to-edge policy, and Gate 7 launch attestation. */
public final class AlfaApplication extends Application {
    ...
    @Override public void onCreate() {
        ...
        registerActivityLifecycleCallbacks(new ActivityLifecycleCallbacks() {
            ...
            @Override public void onActivityStopped(Activity a) {
                startedActivities.updateAndGet(v -> Math.max(0, v - 1));
                if (a instanceof MainActivity) activityPauseInProgress = false;
            }
            @Override public void onActivityCreated(Activity a, Bundle state) {
                installWindowInsetsPolicy(a);
                AlfaUiTheme.apply(a);
    ...
    private static void installWindowInsetsPolicy(Activity a) {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(content, (v, in) -> {
            Insets i = in.getInsets(types);
    ...
    private void installLaunchContract() {
        ...
        File dst = new File(vault, ASSET), tmp = new File(vault, ASSET + ".part");
        try (InputStream in = getAssets().open(ASSET); FileOutputStream out = new FileOutputStream(tmp)) {
            byte[] buf = new byte[4096]; int n;
            while ((n = in.read(buf)) != -1) out.write(buf, 0, n);
            out.getFD().sync();
            if (!tmp.renameTo(dst)) {
                if (dst.exists()) dst.delete();
                if (!tmp.renameTo(dst)) throw new IllegalStateException("launch-contract-publish-failed");
            }
            validate(dst);
    ...
    private static void validate(File f) throws Exception {
        ...
        String[] keys = {"pipeline_run_id", "runtime_id", "source_commit", "gate4_contract_sha256", "profile_sha256", "implementation_commit", "runtime_registry_sha256"};
        for (String k : keys) require(p, k);
        if (RuntimeRegistry.get(p.getProperty("runtime_id")) == null) throw new IllegalStateException("unsupported-runtime-id");
        if (!p.getProperty("source_commit").matches("[0-9a-fA-F]{40}") || !p.getProperty("implementation_commit").matches("[0-9a-fA-F]{40}")) throw new IllegalStateException("invalid-commit");
        for (String k : new String[]{"gate4_contract_sha256", "profile_sha256", "runtime_registry_sha256"}) if (!p.getProperty(k).matches("[0-9a-fA-F]{64}")) throw new IllegalStateException("invalid-hash");
    }
