...
public final class SecurityEvidenceSnapshot {
    ...
    private SecurityEvidenceSnapshot(String packageName,String uid,String selinux,String seccomp,String shizukuInstalled,String overlay,String notifications,String fgs,String runtimeReady){this.packageName=packageName;this.uid=uid;this.selinux=selinux;this.seccomp=seccomp;this.shizukuInstalled=shizukuInstalled;this.overlay=overlay;this.notifications=notifications;this.fgs=fgs;this.runtimeReady=runtimeReady;}
    ...
    public static SecurityEvidenceSnapshot collect(Context context) {
        ...
        try { context.getPackageManager().getPackageInfo("moe.shizuku.privileged.api",0); shizuku="INSTALLED"; } catch (PackageManager.NameNotFoundException ignored) { shizuku="NOT_INSTALLED"; }
        ...
        return new SecurityEvidenceSnapshot(context.getPackageName(),String.valueOf(android.os.Process.myUid()),selinux.isEmpty()?"UNKNOWN":selinux,seccomp.isEmpty()?"UNKNOWN":seccomp,shizuku,Settings.canDrawOverlays(context)?"GRANTED":"DENIED",notifications,RuntimeKeepAliveService.isActive()?"ACTIVE":"INACTIVE",ready);
    }
