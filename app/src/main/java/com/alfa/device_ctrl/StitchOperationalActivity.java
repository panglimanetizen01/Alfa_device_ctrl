...
public final class StitchOperationalActivity extends Activity {
    ...
    private final RuntimeSessionMultiplexer sessionMultiplexer = new RuntimeSessionMultiplexer();
    private TerminalView terminal;
    private TextView systemState,sessionState,terminalState,telemetry,status,sessionList;
    private LinearLayout nodePage,lanePage,diagPage;
    ...
    @Override protected void onCreate(Bundle state){
        ...
        getWindow().setStatusBarColor(BG);getWindow().setNavigationBarColor(Color.rgb(10,14,19));
        setContentView(build());active=true;refreshNodes();refreshSessionSwitcher();
    }
    @Override protected void onStart(){super.onStart();active=true;RuntimeSessionManager manager=activeManager();if(manager!=null)manager.rebindListener(listenerFor(sessionMultiplexer.activeSessionId()));refreshNodes();refreshSessionSwitcher();}
    @Override protected void onStop(){active=false;sessionMultiplexer.stopAll();super.onStop();}
    @Override protected void onDestroy(){active=false;sessionMultiplexer.stopAll();super.onDestroy();}
    ...
    private View buildLanes(){lanePage=col(BG);lanePage.setVisibility(View.GONE);lanePage.addView(window("LANE 01 // SESSION MULTIPLEXER",terminalWindowContent()),new LinearLayout.LayoutParams(-1,0,1));lanePage.addView(sp(8));lanePage.addView(window("LANE 02 // EXECUTION",executionContent()));return lanePage;}
    ...
    private View terminalWindowContent(){
        LinearLayout b=col(Color.rgb(10,14,19));
        LinearLayout head=row(HIGH);head.setGravity(Gravity.CENTER_VERTICAL);
        terminalState=txt("SESSION -- // "+selectedRuntime.displayName(),GREEN,10,true);head.addView(terminalState,new LinearLayout.LayoutParams(0,dp(40),1));
        head.addView(btn("NEW",CYAN,v->createAndStartNextSession()),new LinearLayout.LayoutParams(dp(58),dp(40)));
        head.addView(btn("STOP",RED,v->stopActiveSession()),new LinearLayout.LayoutParams(dp(58),dp(40)));
        head.addView(btn("REMOVE",RED,v->removeActiveSession()),new LinearLayout.LayoutParams(dp(72),dp(40)));
        b.addView(head);
        sessionList=txt("No active session",MUTED,9,true);b.addView(sessionList,new LinearLayout.LayoutParams(-1,dp(34)));
        FrameLayout f=new FrameLayout(this);terminal=new TerminalView(this,null);terminal.setTerminalViewClient(new AlfaTerminalViewClient());f.addView(terminal,new FrameLayout.LayoutParams(-1,-1));
        TextView empty=txt("PTY output appears here after OPEN on a READY runtime.",MUTED,11,false);empty.setGravity(Gravity.CENTER);f.addView(empty,new FrameLayout.LayoutParams(-1,-1));f.setTag(empty);b.addView(f,new LinearLayout.LayoutParams(-1,0,1));
        LinearLayout actions=row(SURFACE);EditText input=new EditText(this);input.setSingleLine(true);input.setHint("command → PTY");input.setTextColor(TEXT);input.setHintTextColor(MUTED);input.setTypeface(Typeface.MONOSPACE);input.setTextSize(11);actions.addView(input,new LinearLayout.LayoutParams(0,dp(46),1));actions.addView(btn("SEND",GREEN,v->writePty(input.getText().toString())),new LinearLayout.LayoutParams(dp(72),dp(46)));actions.addView(btn("APT UPDATE",CYAN,v->writePty("apt update")),new LinearLayout.LayoutParams(dp(104),dp(46)));b.addView(actions);return b;
    }
    private View executionContent(){LinearLayout b=col(SURFACE);b.setPadding(dp(10),dp(8),dp(10),dp(8));b.addView(txt("Session Switcher → selectedSessionId → RuntimeSessionMultiplexer → RuntimeSessionManager → TerminalSession → PTY",TEXT,10,true));b.addView(btn("RUN PING",CYAN,v->writePty("ping -c 1 1.1.1.1")),new LinearLayout.LayoutParams(-1,dp(46)));b.addView(btn("RUN UTS PROBE",GREEN,v->writePty("uname -a && id && pwd")),new LinearLayout.LayoutParams(-1,dp(46)));return b;}
    private View diagnosticContent(){LinearLayout b=col(SURFACE);b.setPadding(dp(10),dp(8),dp(10),dp(8));telemetry=txt("CPU=--  MEM=--  STORAGE=--  NET=--",MUTED,11,true);b.addView(telemetry);status=txt("Evidence channel idle.",MUTED,10,false);status.setTypeface(Typeface.MONOSPACE);b.addView(status);b.addView(btn("REFRESH LIVE TELEMETRY",CYAN,v->refreshTelemetry()),new LinearLayout.LayoutParams(-1,dp(46)));b.addView(btn("CHECK RUNTIME",GREEN,v->refreshNodes()),new LinearLayout.LayoutParams(-1,dp(46)));b.addView(btn("STOP ALL SESSIONS",RED,v->{sessionMultiplexer.stopAll();showStatus("ALL SESSIONS STOP REQUESTED");}),new LinearLayout.LayoutParams(-1,dp(46)));return b;}
    private View stateContent(){LinearLayout b=col(SURFACE);b.setPadding(dp(10),dp(8),dp(10),dp(8));b.addView(txt("READY ⇒ OPEN only. INSTALL is structurally absent while READY.",GREEN,10,true));b.addView(txt("NOT_READY ⇒ INSTALL. READY ⇒ OPEN.",MUTED,10,false));b.addView(txt("Multi-PTY is canonical: every session owns a distinct RuntimeSessionManager/TerminalSession.",CYAN,10,true));return b;}
    ...
    private RuntimeSessionManager activeManager(){return sessionMultiplexer.activeSession();}
    private String nextSessionId(){int n=1;while(sessionMultiplexer.containsSession("SESSION_"+n))n++;return "SESSION_"+n;}
    private void createAndStartNextSession(){if(selectedRuntime==null)return;String id=nextSessionId();createAndStartSession(id);}
    private void createAndStartSession(String sessionId){
        if(!active)return;
        File runtime=new File(new File(getFilesDir(),"runtime-vault"),"runtimes/"+selectedRuntime.id());
        File ready=new File(runtime,"READY.evidence");File launch=new File(new File(getFilesDir(),"runtime-vault"),"gate7-launch.properties");
        if(!Gate6LaunchContract.verify(launch,selectedRuntime.id())){Intent i=new Intent(this,Gate6ImportActivity.class);i.putExtra("runtime_id",selectedRuntime.id());startActivityForResult(i,GATE6);return;}
        File cwd=new File(new File(getFilesDir(),"session-cwd"),sessionId);if(!ready.isFile()||(!cwd.exists()&&!cwd.mkdirs())){showStatus("RUNTIME READY EVIDENCE MISSING");return;}
        try{
            InteractiveSessionContract contract=new InteractiveSessionContract(sessionId,"request-"+id(),"run-"+id(),selectedRuntime.id(),ready,new File(getApplicationInfo().nativeLibraryDir,"libproot.so"),new File(runtime,"rootfs"),cwd,new String[]{"HOME=/root","TERM=xterm-256color","PS1=alfa:"+selectedRuntime.id()+":"+sessionId+":\\w\\$ ","PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin","PROOT_TMP_DIR="+new File(runtime,"proot_tmp").getAbsolutePath()});
            RuntimeSessionManager manager=sessionMultiplexer.createSession(sessionId,contract,listenerFor(sessionId));
            sessionMultiplexer.setActiveSessionId(sessionId);
            if(!manager.start(80,24,8,16)){sessionMultiplexer.removeSession(sessionId);showStatus("PTY START BLOCKED // "+sessionId);return;}
            manager.attachTo(terminal);hideTerminalOverlay();refreshSessionSwitcher();showStatus("PTY READY // "+selectedRuntime.displayName()+" // "+sessionId);
        }catch(RuntimeException e){showStatus("PTY START ERROR // "+e.getClass().getSimpleName()+":"+e.getMessage());}
    }
    private void startSession(){if(activeManager()!=null&&activeManager().isRunning()){activeManager().attachTo(terminal);return;}if(sessionMultiplexer.size()==0)createAndStartSession("SESSION_1");else{String id=sessionMultiplexer.activeSessionId();if(id==null)createAndStartSession(nextSessionId());else {sessionMultiplexer.setActiveSessionId(id);activeManager().attachTo(terminal);}}}
    private RuntimeSessionManager.Listener listenerFor(final String sessionId){return new RuntimeSessionManager.Listener(){@Override public void onState(String s){runOnUiThread(()->{if(sessionId.equals(sessionMultiplexer.activeSessionId())){sessionState.setText("PTY: "+s);terminalState.setText(sessionId+" // "+selectedRuntime.displayName()+" // "+s);systemState.setText(("READY".equals(s)||"RUNNING".equals(s))?"SYSTEM ONLINE":"SYSTEM WAITING");}}refreshSessionSwitcher();});}@Override public void onTextChanged(){runOnUiThread(()->{if(sessionId.equals(sessionMultiplexer.activeSessionId())&&terminal!=null)terminal.invalidate();});}@Override public void onSessionFinished(int code){runOnUiThread(()->{if(sessionId.equals(sessionMultiplexer.activeSessionId()))sessionState.setText("PTY: FINISHED "+code);refreshSessionSwitcher();});}};}
    private void switchSession(String sessionId){if(!sessionMultiplexer.containsSession(sessionId))return;sessionMultiplexer.setActiveSessionId(sessionId);RuntimeSessionManager manager=activeManager();if(manager!=null&&manager.isRunning()){manager.attachTo(terminal);hideTerminalOverlay();}refreshSessionSwitcher();showStatus("SESSION SWITCH // "+sessionId);}
    private void refreshSessionSwitcher(){if(sessionList==null)return;StringBuilder b=new StringBuilder("ACTIVE="+String.valueOf(sessionMultiplexer.activeSessionId())+"  |  ");for(String id:sessionMultiplexer.sessionIds()){b.append(id).append(" ");}sessionList.setText(b.toString());}
    private void stopActiveSession(){String id=sessionMultiplexer.activeSessionId();if(id!=null)sessionMultiplexer.stopSession(id);sessionState.setText("PTY: OFFLINE // "+String.valueOf(id));refreshSessionSwitcher();}
    private void removeActiveSession(){String id=sessionMultiplexer.activeSessionId();if(id==null)return;sessionMultiplexer.removeSession(id);RuntimeSessionManager manager=activeManager();if(manager!=null&&manager.isRunning()){manager.attachTo(terminal);hideTerminalOverlay();}else if(terminal!=null)terminal.setVisibility(View.VISIBLE);refreshSessionSwitcher();showStatus("SESSION REMOVED // "+id);}
    ...
    private void restart(){String id=sessionMultiplexer.activeSessionId();if(id==null){startSession();return;}sessionMultiplexer.stopSession(id);new android.os.Handler().postDelayed(()->startSession(),250);}
    private void writePty(String command){if(command==null||command.trim().isEmpty())return;RuntimeSessionManager manager=activeManager();TerminalSession s=manager==null?null:manager.currentSession();if(s==null||!s.isRunning()){showStatus("PTY NOT READY");return;}byte[] bytes=(command+"\n").getBytes(StandardCharsets.UTF_8);s.write(bytes,0,bytes.length);showStatus("PTY WRITE // "+sessionMultiplexer.activeSessionId()+" // "+command);}
    private void refreshTelemetry(){RuntimeSessionManager manager=activeManager();if(manager==null||!manager.isPromptReady()){showStatus("TELEMETRY BLOCKED // PTY NOT READY");return;}manager.runRuntimeCommand("awk '/MemTotal:/{print \"MEM=\"$2}' /proc/meminfo; df -P / | awk 'NR==2{print \"STORAGE=\"$5}'",(out,code)->runOnUiThread(()->{telemetry.setText("EXIT="+code+"\\n"+out);status.setText(out);}));}
