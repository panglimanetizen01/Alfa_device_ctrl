...
/**
 * Native state model for the supplied Stitch v1 catalog. It maps every catalog state
 * to an existing Alfa operational surface without inventing backend capabilities.
 */
public final class StitchV1StateModel {
    public enum Domain { TERMINAL, RUNTIME, NETWORK, SECURITY, STORAGE_POLICY, PROJECT_STORAGE, SESSIONS, SPLIT, FLOATING, SETTINGS, OTHER }
    ...
    public static final class State {
        ...
        private final int supportedActionCount;

        private State(String id, Domain domain, AlfaUiNavigation.Screen primaryScreen,
                      boolean referenceOnly, boolean claimsLiveExecution, int supportedActionCount) {
            this.id = id;
            this.domain = domain;
            this.primaryScreen = primaryScreen;
            this.referenceOnly = referenceOnly;
            this.claimsLiveExecution = claimsLiveExecution;
            this.supportedActionCount = supportedActionCount;
        }
        ...
        public int supportedActionCount() { return supportedActionCount; }
    }
    ...
    public static State resolve(String id) {
        ...
        boolean simulation = s.contains("simulation");
        boolean referenceOnly = simulation || s.contains("maximum_sessions_reached") || s.contains("primary_app_icon") || s.contains("app_icon_suite_presentation");
        boolean claimsLiveExecution = !referenceOnly && (domain == Domain.TERMINAL || domain == Domain.RUNTIME || domain == Domain.NETWORK || domain == Domain.SESSIONS || domain == Domain.SPLIT || domain == Domain.FLOATING);
        return new State(id, domain, screenOf(domain), referenceOnly, claimsLiveExecution, actionCount(s, domain));
    }
    ...
    private static int actionCount(String s, Domain domain) {
        int count = 1;
        if (s.contains("dialog") || s.contains("confirmation") || s.contains("settings") || s.contains("picker")) count++;
        if (s.contains("export") || s.contains("import") || s.contains("backup") || s.contains("audit")) count++;
        if (domain == Domain.SESSIONS || domain == Domain.SPLIT || domain == Domain.FLOATING) count++;
        if (domain == Domain.NETWORK || domain == Domain.SECURITY) count++;
        return count;
    }
}
