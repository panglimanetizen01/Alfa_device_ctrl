 ...
 *
 * The catalog is an implementation input: each state resolves to a native
 * Alfa surface and, where the state describes an executable operation, to an
 * explicit action. Result/simulation states are never promoted to fabricated
 * live evidence.
 */
public final class StitchV1StateModel {
    public enum Domain {
        TERMINAL, RUNTIME, NETWORK, SECURITY, STORAGE_POLICY, PROJECT_STORAGE,
        AUDIT, SESSIONS, SPLIT, FLOATING, SETTINGS, OTHER
    }
    ...
    public static State resolve(String id) {
        ...
        boolean claimsLiveExecution = !referenceOnly &&
                (domain == Domain.TERMINAL || domain == Domain.RUNTIME || domain == Domain.SESSIONS);
    ...
    private static Domain domainOf(String s) {
        ...
        if (s.contains("network") || s.contains("socket") || s.contains("tunnel") ||
                s.contains("matrix") || s.contains("dns")) return Domain.NETWORK;
        if (s.contains("mount") || s.contains("override") || s.contains("sensitive_files") ||
                s.contains("policy") || s.contains("filesystem_bind")) return Domain.STORAGE_POLICY;
        if (s.contains("audit") || s.contains("share_audit")) return Domain.AUDIT;
        if (s.contains("project") || s.contains("file_browser") || s.contains("create_new_file") ||
                s.contains("create_new_folder") || s.contains("directory") || s.contains("folder_audit_scripts")) {
            return Domain.PROJECT_STORAGE;
        }
        if (s.contains("appearance") || s.contains("font") || s.contains("language") ||
                s.contains("settings") || s.contains("control_center") || s.contains("pengaturan_bahasa")) {
        ...
        if (s.contains("runtime") || s.contains("linux_3.24") || s.contains("ubuntu_24.04") ||
                s.contains("kali_nethunter")) return Domain.RUNTIME;
        if (s.contains("terminal") || s.contains("operational") || s.contains("termux_open_ingestion")) {
    ...
    private static AlfaUiNavigation.Screen screenOf(Domain domain) {
        switch (domain) {
            ...
            case AUDIT: return AlfaUiNavigation.Screen.AUDIT;
