...
public final class RuntimeProfile {
    ...
    /**
     * Minimal compatibility bindings required by Alfa's current PTY/process model.
     * /sys is deliberately not exposed: it is not required to create the PTY or
     * run the supported shell, while binding the complete host tree increases the
     * host-information/device surface visible to the guest.
     */
    private static String[] defaultProotArguments(){return new String[]{"-0","-r","{RUNTIME_ROOT}","-b","/dev","-b","/proc","-w","/root","/usr/bin/env","-i","HOME=/root","PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin","TERM=xterm-256color","PS1={PROMPT}","{SHELL}","-i"};}
