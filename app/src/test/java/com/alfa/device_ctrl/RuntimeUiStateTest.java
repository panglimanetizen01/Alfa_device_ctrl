...
public final class RuntimeUiStateTest {
    ...
    @Test public void runtimeDirectoryWithoutRootfsIsPartial() throws Exception {
        File root = new File(System.getProperty("java.io.tmpdir"), "alfa-ui-partial-" + System.nanoTime());
        assertEquals(true, root.mkdirs());
        assertEquals(RuntimeUiState.Status.PARTIAL,
                RuntimeUiState.resolve("debian", root, new File(root, "READY.evidence"), new File(root, "libproot.so"), new File(root, "rootfs")));
    }
    ...
    @Test public void evidenceWithoutExecutableEngineFailsClosed() throws Exception {
        File root = new File(System.getProperty("java.io.tmpdir"), "alfa-ui-engine-" + System.nanoTime());
        assertEquals(true, root.mkdirs());
        File evidence = new File(root, "READY.evidence");
        try (FileWriter writer = new FileWriter(evidence)) { writer.write("status=READY\n"); }
        File rootfs = new File(root, "rootfs");
        assertEquals(true, rootfs.mkdirs());
        File engine = new File(root, "libproot.so");
        assertEquals(true, engine.createNewFile());
        assertEquals(RuntimeUiState.Status.FAILED,
                RuntimeUiState.resolve("debian", root, evidence, engine, rootfs));
    }
}
