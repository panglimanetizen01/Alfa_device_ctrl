...
public final class StitchV1StateModelContractTest {
    ...
    @Test
    public void unsupported_split_and_floating_states_never_claim_live_execution() {
        for (String id : StitchV1ReferenceCatalog.screenIds()) {
            StitchV1StateModel.State state = StitchV1StateModel.resolve(id);
            if (state.domain() .equals("split") || state.domain().equals("floating")) {
                assertFalse(id, state.claimsLiveExecution());
            }
        }
    }
    ...
    public void stateDomainsCoverTheOperationalSurface() {
        ...
        assertEquals("audit", StitchV1StateModel.resolve("alfa_device_ctrl_audit_log_seccomp_mount_filtered").domain());
        assertEquals("security", StitchV1StateModel.resolve("alfa_device_ctrl_syscall_trap_verification_dialog").domain());
    }
}
