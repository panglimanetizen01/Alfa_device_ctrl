...
public final class RuntimeSmokePtraceDiagnosticTest {
    ...
    @Test public void capturePtraceStateUntilSmokeBoundary() throws Exception {
        Context c = InstrumentationRegistry.getInstrumentation().getTargetContext();
        File vault = new File(c.getFilesDir(), "runtime-vault-ptrace-diagnostic"); delete(vault);
        ...
        File evidence = new File(c.getFilesDir(), "g2-proot-ptrace-diagnostic.txt"); delete(evidence);
        RuntimeInstaller installer = new RuntimeInstaller(vault, m -> Log.i(TAG, m), engine, lib, (e, root) -> runDiagnostic(e, root, evidence));
        RuntimeInstaller.Result result = installer.install(profile, null, RuntimeInstaller.TRUSTED_PROOT_ARM64_SHA256, new URL(profile.rootfsUrl()), profile.rootfsSha256(), profile.rootfsGzip());
    ...
    private static String runDiagnostic(File engine, File root, File evidence) {
        java.lang.Process p = null; long start = System.nanoTime();
        try {
            File tmp = new File(root.getParentFile(), "proot_tmp"), loader = new File(engine.getParentFile(), "libproot-loader.so");
            List<String> cmd = new ArrayList<>(Arrays.asList(engine.getAbsolutePath(), "-0", "-r", root.getAbsolutePath(), "-b", "/dev", "-b", "/proc", "-b", "/sys", "-w", "/root", "/usr/bin/env", "-i", "HOME=/root", "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin", "TERM=xterm-256color", "PROOT_TMP_DIR=" + tmp.getAbsolutePath(), "PROOT_NO_SECCOMP=1", "/bin/sh", "-c", "printf 'ALFA_RUNTIME_SMOKE_OK\\n'; id -u; pwd"));
            ProcessBuilder b = new ProcessBuilder(cmd); b.environment().put("PROOT_TMP_DIR", tmp.getAbsolutePath()); b.environment().put("PROOT_LOADER", loader.getAbsolutePath()); b.environment().put("PROOT_NO_SECCOMP", "1"); b.environment().put("PROOT_VERBOSE", "9"); b.directory(root.getParentFile()); b.redirectErrorStream(true);
            File out = new File(evidence.getParentFile(), "g2-proot-verbose.log"); b.redirectOutput(out); p = b.start();
            int ppid = android.os.Process.myPid(); write(evidence, "parent_test_pid=" + ppid + "\nengine=" + engine + "\nroot=" + root + "\nstart_ns=" + start + "\n");
            long deadline = System.nanoTime() + TimeUnit.SECONDS.toNanos(35);
            while (p.isAlive() && System.nanoTime() < deadline) { long ms = TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - start); append(evidence, "\n=== sample_ms=" + ms + " process_alive=" + p.isAlive() + " ===\n"); List<Integer> roots = children(ppid); append(evidence, "test_children=" + roots + "\n"); for (int pid : roots) dumpTree(pid, evidence, 0, new HashSet<>()); Thread.sleep(250); }
            boolean alive = p.isAlive(); append(evidence, "\n=== boundary alive=" + alive + " elapsed_ms=" + TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - start) + " ===\n");
