...
/** G17 proof: the exact Gate16 authorization is executed by the Android app runtime. */
public final class G17AndroidRuntimeExecutionTest {
    ...
    @Test public void exactGate16AuthorizationExecutesPwdInsidePackagedRuntime() throws Exception {
        ...
        assertTrue(engine.isFile() && engine.canExecute());
        assertEquals(PROOT_SHA256, sha256(engine));

        int uid = android.os.Process.myUid();
        assertEquals(c.getPackageManager().getApplicationInfo(PACKAGE, 0).uid, uid);
        assertTrue(uid != 2000);
        String selinux = line(new File("/proc/self/attr/current"));
        assertTrue(!selinux.isEmpty() && !"u:r:shell:s0".equals(selinux));
        ...
        assertTrue(ready.isFile() && rootfs.isDirectory());
        for (String d : new String[]{"proc", "dev", "sys"}) assertTrue(new File(rootfs, d).isDirectory() || new File(rootfs, d).mkdirs());
        assertTrue(RuntimeEvidence.verify(ready, "ubuntu", engine, rootfs));

        String source = arg("source_commit");
        File g16 = new File(arg("gate16_path"));
        assertTrue(g16.isFile());
        ...
        for (String k : new String[]{"gate4_contract_sha256","profile_sha256","gate15_policy_sha256","gate5_authorization_sha256"}) assertTrue(SHA256.matcher(a.getProperty(k, "")).matches());

        String script = "set -eu; printf 'G17_GUEST_PROC_EXE='; readlink /proc/self/exe; printf 'G17_GUEST_PROC_CWD='; readlink /proc/self/cwd; printf 'G17_GUEST_PROC_ROOT='; readlink /proc/self/root; printf 'G17_GUEST_PID=%s\\n' \"$$\"; printf 'G17_COMMAND_RESULT='; pwd; printf 'G17_COMMAND_RETURNCODE=0\\n'";
        List<String> x = new ArrayList<>();
        Collections.addAll(x, engine.getAbsolutePath(), "-0", "-r", rootfs.getAbsolutePath(), "-b", "/dev", "-b", "/proc", "-b", "/sys", "-w", "/root", "/usr/bin/env", "-i",
                "HOME=/root", "PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin", "TERM=xterm-256color", "PROOT_TMP_DIR=" + new File(runtime, "proot_tmp").getAbsolutePath(), "/bin/sh", "-c", script);
        java.lang.Process p = new ProcessBuilder(x).redirectErrorStream(true).start();
        String output = read(p); int rc = p.waitFor();
        assertEquals(output, 0, rc);
        String exe = field(output,"G17_GUEST_PROC_EXE"), cwd = field(output,"G17_GUEST_PROC_CWD"), root = field(output,"G17_GUEST_PROC_ROOT"), pid = field(output,"G17_GUEST_PID"), result = field(output,"G17_COMMAND_RESULT");
        assertTrue(pid.matches("[0-9]+")); assertTrue(exe.startsWith("/")); assertTrue(!root.isEmpty()); assertEquals("/root", cwd); assertEquals("/root", result);

        File out = new File(c.getFilesDir(), "g17/gate17-runtime-execution.v1"); out.getParentFile().mkdirs();
        try (Writer w = new OutputStreamWriter(new FileOutputStream(out), StandardCharsets.UTF_8)) {
            w.write("schema_version=gate17-runtime-execution.v1\n"); w.write("gate=gate17\n"); w.write("gate_status=PASS\nexecution_status=EXECUTED\nresult_status=PASS\ncommand=pwd\ncommand_semantics=POSIX_PWD\ncommand_result=/root\ncommand_returncode=0\n");
            w.write("guest_pid="+pid+"\nguest_proc_exe="+exe+"\nguest_proc_cwd="+cwd+"\nguest_proc_root="+root+"\nandroid_package="+PACKAGE+"\nandroid_uid="+uid+"\nandroid_selinux_context="+selinux+"\n");
            w.write("execution_engine="+engine.getCanonicalPath()+"\nengine_sha256="+sha256(engine)+"\nrootfs_path="+rootfs.getCanonicalPath()+"\nrootfs_os_release_sha256="+sha256(new File(rootfs,"etc/os-release"))+"\npipeline_run_id="+a.getProperty("pipeline_run_id")+"\nsource_commit="+source+"\ngate4_contract_sha256="+a.getProperty("gate4_contract_sha256")+"\nprofile_sha256="+a.getProperty("profile_sha256")+"\ngate16_authorization_sha256="+sha256(g16)+"\ngate16_authorization_id="+a.getProperty("authorization_id")+"\ntimestamp="+System.currentTimeMillis()+"\n");
        }
        System.out.println("G17_LIVE_STATUS=PASS"); System.out.println("G17_LIVE_RESULT=REAL_ANDROID_DUT_EXECUTION"); System.out.println("G17_LIVE_PACKAGE="+PACKAGE); System.out.println("G17_LIVE_UID="+uid); System.out.println("G17_LIVE_SELINUX="+selinux); System.out.println("G17_LIVE_ARTIFACT="+out);
    }
