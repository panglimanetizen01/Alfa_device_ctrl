...
grep -Fqx 'PROOT_EXEC=${4:-}; ROOTFS=${5:-}' <(tr '\n' ' ' < "$ROOT/tools/gate17_runtime_execution.sh") || { grep -Fq 'PROOT_EXEC=${4:-}' "$ROOT/tools/gate17_runtime_execution.sh"; grep -Fq 'ROOTFS=${5:-}' "$ROOT/tools/gate17_runtime_execution.sh"; }
grep -Fq '"$PROOT_EXEC" -r "$ROOTFS" -w /root /bin/sh -c "$GUEST_EVIDENCE_SCRIPT"' "$ROOT/tools/gate17_runtime_execution.sh"
...
[ "$(awk -F= '$1=="guest_pid"{print $2;exit}' "$OUT")" = "$(awk -F= '$1=="guest_internal_pid"{print $2;exit}' "$OUT")" ]
