...
G13_ARTIFACT=$(field gate13_artifact "$INPUT")
G12_SHA=$(field gate12_kernel_sha256 "$INPUT")
G12_ARTIFACT=$(field gate12_artifact "$INPUT")
...
[ -f "$G13_ARTIFACT" ] || blocked 'referenced Gate 13 artifact missing'
[ -f "$G12_ARTIFACT" ] || blocked 'referenced Gate 12 artifact missing'
ACTUAL_G13_SHA=$(sha256sum "$G13_ARTIFACT" 2>/dev/null | awk '{print $1}')
ACTUAL_G12_SHA=$(sha256sum "$G12_ARTIFACT" 2>/dev/null | awk '{print $1}')
[ "$ACTUAL_G13_SHA" = "$G13_SHA" ] || blocked 'Gate 13 artifact hash mismatch'
[ "$ACTUAL_G12_SHA" = "$G12_SHA" ] || blocked 'Gate 12 artifact hash mismatch'
