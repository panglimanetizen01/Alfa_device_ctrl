...
main() {
    ...
    RUN_ID=${1:-}
    RUNTIME_ID=${2:-}
    if [ -z "$RUN_ID" ] || [ -z "$RUNTIME_ID" ]; then
        printf '%s\n' 'GATE6_STATUS=BLOCKED'
        printf '%s\n' 'GATE6_REASON=explicit pipeline_run_id and runtime_id are required'
        return 1
    fi
    ...
    STATUS=PASS
    REASON='Gate 4 VALID, Gate 5 decision ALLOW, authorization AUTHORIZED, runtime-bound bootstrap probe passed'
    PROBE=PASS
    NOW=$(chain_now)
